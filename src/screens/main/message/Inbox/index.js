/* eslint-disable no-shadow */
/* eslint-disable prettier/prettier */
/* eslint-disable react-native/no-inline-styles */
import React, { useState, useEffect, useCallback } from "react"
import {
  View,
  Text,
  Image,
  SafeAreaView,
  Button,
  TextInput,
  TouchableOpacity
} from "react-native"
import {
  Bubble,
  GiftedChat,
  Send,
  InputToolbar
} from "react-native-gifted-chat"
// import SendIcon from "../../../../assets/icons/send-chat.svg"
import styles from "./style"
import { BaseColor } from "../../../../config"
import { usePubNub } from "pubnub-react"
import {
  ComingSoon,
  Header,
  HorizontalSpacing,
  VerticalSpacing
} from "../../../../components"

import BackIcon from "../../../../assets/icons/back.svg"
import AddIcon from "../../../../assets/icons/plus.svg"
import { inbox } from "../../../../util/data"
import { useDispatch, useSelector } from "react-redux"
import { setChannel, setActiveChatTitle } from "../../../../redux/reducers/chat"
import { setScreensFlow } from "../../../../redux/reducers/nav"
import { convertTime } from "../../../../util/helpers"

const ChatRow = ({
  title,
  message,
  time,
  profilePicture,
  isOnline,
  openChat
}) => {
  return (
    <TouchableOpacity onPress={openChat} style={styles.chatRowContainer}>
      <View style={styles.chatRowInnerContainer}>
        <Image
          source={{
            uri: profilePicture
          }}
          resizeMode="contain"
          style={styles.chatRowImage}
        />
        <View style={styles.chatRowTextContainer}>
          <Text style={styles.chatRowTitle}>{title}</Text>
          <Text style={styles.chatRowMessage}>{message}</Text>
        </View>
        <Text style={styles.chatRowTime}>{time}</Text>
      </View>
    </TouchableOpacity>
  )
}

const InboxScreen = props => {
  const [messages, setMessages] = useState([])
  const pubnub = usePubNub()
  const dispatch = useDispatch()
  const inboxedChannels = useSelector(state => state.chat.inboxedChannels)

  useEffect(() => {}, [])
  const _handleChatNav = (channelId, activeChatTitle) => {
    props.navigation.navigate("Chat")
    dispatch(setChannel(channelId))
    dispatch(setActiveChatTitle(activeChatTitle))
  }
  const createNew = () => {
    dispatch(
      setScreensFlow({
        screensFlow: "notifyProviders",
        previousScreen: "DashboardNav",
        nextScreen: "Inbox"
      })
    )
    props.navigation.navigate("MainNav", {
      screen: "Interest"
    })
  }
  return (
    <>
      <SafeAreaView>
        <Header
          leftIcon={<BackIcon />}
          title={"chat history".toUpperCase()}
          leftHandler={() => props.navigation.goBack()}
          rightIcon={<AddIcon />}
          rightHandler={createNew}
        />
      </SafeAreaView>
      <HorizontalSpacing
        width="100%"
        height={3}
        backgroundColor={styles.chatRowContainer.borderColor}
      />
      {inboxedChannels.map((channel, index) => {
        console.log(channel)
        const message = {
          porfile:
            "data:image/png;base64,",
          title: "Elmer James",
          message: "Hey, I can solve your problem via zoom call ...",
          time: "06:41 pm",
          isOnline: true
        }
        let lastMesssageTime = "N/A"
        try {
          lastMesssageTime = new Date(
            channel?.custom?.lastMessage?.timetoken / 10000
          )
          if (lastMesssageTime) {
            lastMesssageTime = lastMesssageTime.toString()
            lastMesssageTime = lastMesssageTime.split(" ")[4]
            lastMesssageTime = lastMesssageTime.slice(0, 5)
            lastMesssageTime = convertTime(lastMesssageTime)
          }
        } catch (error) {
          console.log(error)
        }
        return (
          <ChatRow
            key={index}
            title={channel?.custom?.providerName}
            profilePicture={message.porfile}
            message={channel?.custom?.lastMessage?.message?.text}
            time={lastMesssageTime}
            openChat={() =>
              _handleChatNav(channel?.id, channel?.custom?.providerName)
            }
          />
        )
      })}
      {/* <ComingSoon /> */}
      <SafeAreaView></SafeAreaView>
    </>
  )
}

export default InboxScreen
